...
<div class="container my-5">
    <div class="row">
        <div class="col-12 col-md-6 ">
            <div>
                <img src="{{$feature->photo}}" alt="" class="img-fluid w-100 rounded my-2 h-100">
            </div>
           
        </div>
        <div class="col-12 col-md-6 mt-2">
            <h2 class="mb-md-3">{{$feature->name}}</h2>
            <p>Category : <span class="colorcode">{{$feature->category->name}}</span></p>
            <hr>
            <h4 class="colorcode my-2">{{$feature->price - ($feature->discount/100)*$feature->price}}MMK
                <span class="text-secondary text-decoration-line-through me-2">{{$feature->price}}MMK</span>
                <span class="text-danger">{{$feature->discount}}% Off</span>
            </h4>
            <hr>
            ...
            <div class="mt-3 addCart">
                ...
                <button class="shopnow btn cartBtn" data-id="{{$feature->id}}" data-name="{{$feature->name}}" data-image="         {{$feature->photo}}" data-price="{{$feature->price}}" data-discount="{{$feature->discount}}">Add to Cart</button>
            </div>
             <hr>
             <p>CodeNo - {{$feature->codeNo}}</p>
        </div>
    </div>
</div>
<!-- Related Post -->
<div class="container my-5">
    <div class="row">
        ...
        @foreach($feature_detail as $feature)
             ...
             <a href="{{route('featuredetail',$feature->id)}}"><img src="{{$feature->photo}}" alt="" class="card-img-top product_border"></a>
              <div class="card-body pb-0">
                ...
                <span class="text-warning Daisy">
                    ...
                    <button class="btn addcart cartBtn mt-3" data-id="{{$feature->id}}" data-name="{{$feature->name}}" data-image="{{$feature->photo}}" data-price="{{$feature->price}}" data-discount="{{$feature->discount}}">
